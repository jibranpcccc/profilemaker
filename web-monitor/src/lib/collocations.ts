// StealthHumanizer - Collocation Replacements (150+ entries)
...
export const COLLOCATIONS: Collocation[] = [
  ...
  { from: 'streamline', to: ['simplify', 'make smoother', 'speed up'] },
...
export function applyCollocations(text: string): string {
  ...
  for (const col of COLLOCATIONS) {
    ...
    if (regex.test(result)) {
      const replacement = randomPick(col.to);
...
export function applyRandomCollocation(text: string): string {
  const applicable = COLLOCATIONS.filter(col =>
    text.toLowerCase().includes(col.from.toLowerCase())
  );
  if (applicable.length === 0) return text;

  const col = randomPick(applicable);
  const replacement = randomPick(col.to);
  const regex = new RegExp(col.from.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi');
